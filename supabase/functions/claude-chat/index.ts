...
async function queryFlowise(data: any) {
  console.log("Calling Flowise with data:", JSON.stringify(data));
  ...
  if (!response.ok) {
    ...
    throw new Error(`Flowise returned ${response.status}: ${errorText}`);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Starting Flowise API call with message:",
    ...
    // Make the API request to Flowise
    ...
    if (response.sessionId) {
      ...
      console.log("Received session ID from Flowise:", returnedSessionId);
    }

    console.log("Final response text extracted:", responseText.substring(0, 50) + "...");
    ...
    console.error("Error in Flowise chat function:", error);
    return new Response(
